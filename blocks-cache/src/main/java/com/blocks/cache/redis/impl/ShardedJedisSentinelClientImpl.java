...
import com.blocks.cache.redis.AbstractSharededJedisSentinelClient;
 ...
 */
public class ShardedJedisSentinelClientImpl extends AbstractSharededJedisSentinelClient implements ShardedJedisSentinelClient {

    private final Logger logger = Logger.getLogger(getClass().getName());

    /**
     * 自定义配置文件方式初始化sentinel连接池
     * @param configPath
     */
    public ShardedJedisSentinelClientImpl(String configPath) {
        super(configPath);
    }

    /**
     * spring配置方式初始化sentinel连接池
     * @param masters
     * @param sentinels
     */
    public ShardedJedisSentinelClientImpl(List<String> masters, Set<String> sentinels){
        super(masters, sentinels);
    }

    public ShardedJedisSentinelClientImpl(GenericObjectPoolConfig poolConfig, List<String> masters, Set<String> sentinels){
        super(poolConfig, masters, sentinels);
    }

    public void destory() {
        if (null != shardedJedisSentinelPool){
    ...
    public <T> T execute(ShardedJedisAction<T> action) {
        ...
        try{
            jedis = shardedJedisSentinelPool.getResource();
            ...
            if (null != jedis){
                try {
                    shardedJedisSentinelPool.returnResource(jedis);
                } catch(Exception ex) {
    ...
    public void subscrible(final JedisMessageListener pubSubListener, final String... channels) {
        new Thread(new Thread(){
            ...
            public void run() {
                ...
                ShardedJedis shardedJedis = shardedJedisSentinelPool.getResource();
